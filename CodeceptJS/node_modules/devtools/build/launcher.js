...
import { CHROME_NAMES, FIREFOX_NAMES, EDGE_NAMES, BROWSER_TYPE, DEFAULT_FLAGS, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_X_POSITION, DEFAULT_Y_POSITION, VENDOR_PREFIX } from './constants.js';
...
async function launchChrome(capabilities) {
    ...
    const chrome = await launchChromeBrowser(launchOptions).catch((err) => launchChromeUsingWhich(err, launchOptions));
...
export default async function launch(capabilities) {
    try {
    ...
    const devtoolsOptions = capabilities['wdio:devtoolsOptions'] || {};
    const connectionUrl = ((browserOptions?.debuggerAddress && `http://${browserOptions?.debuggerAddress}`) ||
